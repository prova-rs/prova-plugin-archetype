# prova-init-package-archetype

An [Archetect](https://github.com/archetect/archetect) archetype that scaffolds a
[Prova](https://github.com/prova-rs/prova) **package** that acts as a **package** — in Prova a package
...
package shape, but its `prova.toml` also wears the `[package]` hat and it ships an entry module plus a
self-test.
...
(or `archetect render https://github.com/prova-rs/prova-init-package-archetype.git` directly).

## Two shapes, one core

Every package gets the same **core** — `init.lua`, the dual-role `prova.toml`, a `library/<name>.lua`
...
- **Local** (the default inside an existing prova package): the core alone, rendered into the
  package's `packages` (e.g. `.prova/packages/<name>/`), where `require("<name>")` reaches it from
...
the package's root/`packages` whenever the cwd is inside a package (its generic state injection;
...
```
prova-<name>/                     # standalone            .prova/packages/<name>/   # local
├── init.lua                      # the namespace         ├── init.lua
├── prova.toml                    # [package] + self-suite ├── prova.toml
...
A local package is a real package (its `prova.toml` makes it a nested-package boundary, so its proofs
stay out of the owning project's suite — run them from inside its directory). Graduating it to a
shared repo later is `prova init package -s standalone` in a fresh directory plus moving the core
across.

## Kind-agnostic by design

The scaffold commits to no particular kind of package. `init.lua` returns a namespace with a sample
...
- **A topology** — a whole environment `prova up` stands up (`[[package.topologies]]`), gated on the
  tools it needs.

Flesh out `init.lua`, prove it in `proofs/`, and for a standalone package:
...
package's own suite and `prova package lint` (see `proofs/render_test.lua`).
